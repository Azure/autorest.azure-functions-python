...
class CodeGenerator(Plugin):
        ...
        for group in yaml_data["operationGroups"]:
            for operation in group["operations"]:
                ...
                while i < len(operation["exceptions"]):
                    ...
                    if exception.get("schema") and \
                            exception["schema"]["language"]["default"][
                                "name"] == "CloudError":
    ...
    def _create_code_model(self, yaml_data: Dict[str, Any],
                           options: Dict[str, Union[str, bool]]) -> CodeModel:
        # Create a code model
        ...
        code_model.global_parameters = ParameterList(
                [Parameter.from_yaml(param) for param in
                 yaml_data.get("globalParameters", [])],
                implementation="Client",
        )

        # Custom URL
        dollar_host = [parameter for parameter in code_model.global_parameters
                       if parameter.rest_api_name == "$host"]
        if not dollar_host:
            # We don't want to support multi-api customurl YET (will see if
            # that goes well....)
            ...
            code_model.base_url = dollar_host_parameter.yaml_data[
                "clientDefaultValue"]

        # Create operations
        code_model.operation_groups = [
            OperationGroup.from_yaml(code_model, op_group) for op_group in
        ...
        if yaml_data.get("schemas"):
            ...
            # sets the enums property in our code_model variable, which will
            # later be passed to EnumSerializer
    ...
    def _build_code_model_options(self) -> Dict[str, Any]:
        ...
        if self._autorestapi.get_boolean_value("credential-scopes",
                                               False) and not credential_scopes:
            raise ValueError(
                    "--credential-scopes takes a list of scopes in comma "
                    "separated format. "
                    "For example: "
        ...
        if not credential_scopes:
            if azure_arm:
                ...
                # If add-credential is specified, we still want to add a
                # credential_scopes variable.
                # Will make it an empty list so we can differentiate between
                # this case and None
                _LOGGER.warning(
                        "You have used the --add-credential flag but not the "
                        "--credential-scopes flag "
                        "while generating non-management plane code. "
                        "This is not recommend because it forces the customer "
                        "to pass credential scopes "
    ...
    def process(self) -> bool:
        ...
        code_model = self._create_code_model(yaml_data=yaml_data,
                                             options=options)
