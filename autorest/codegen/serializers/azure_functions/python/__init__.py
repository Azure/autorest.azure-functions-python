...
__all__ = [
    "AzureFunctionsPythonSerializer",
...
class AzureFunctionsPythonSerializer(AzureFunctionsSerializer):
    ...
    def serialize(self):
        ...
        self._serialize_and_write_functions(
    ...
    def _serialize_and_write_models_folder(self, code_model: CodeModel,
        ...
        if code_model.schemas:
            self.autorest_api.write_file(
                    ...
                    ModelGenericSerializer(code_model=code_model,
                                           env=env).serialize()
            )
            self.autorest_api.write_file(
                    models_path / Path("_models_py3.py"),
                    ModelPython3Serializer(code_model=code_model,
                                           env=env).serialize()
            )
        if code_model.enums:
            self.autorest_api.write_file(
    ...
    def _serialize_and_write_function_app_contents(
        ...
        azure_functions_serializer = AzureFunctionsPythonAppSerializer(
        ...
        self.autorest_api.write_file(
                function_app_path /
                Path(".gitignore"),
                azure_functions_serializer.serialize_gitignore_file()
        )
...
class AzureFunctionsPythonAppSerializer(object):
